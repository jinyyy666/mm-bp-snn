...
/*
* function: intrinstic plasticity
* blocks  : dim3(batch, outputAmount)
* threads : dim3(min(1024, outputSize))
*/
__global__ void g_intrinsic_plasticity(int * batchFireCount, float* tauTmp, float* resTmp, float* _tau, float * _res, int endTime, int outputArea, int outputSize, int T_REFRAC, float vth, float u)
{
    int batchId = blockIdx.x;
    int ok = blockIdx.y;
    int * fireCount = batchFireCount + ok * outputArea + batchId * outputSize;
    float * tau = _tau + ok * outputSize;
    float * res = _res + ok * outputSize;
    float * taugrad = tauTmp + ok * outputArea + batchId * outputSize;
    float * resgrad = resTmp + ok * outputArea + batchId * outputSize;
    
    for(int tidx = 0; tidx < outputSize; tidx += blockDim.x)
    {
        int o_idx = tidx + threadIdx.x;
        if(o_idx < outputSize){
            float y = float(fireCount[o_idx]) / endTime;
            if(y < 0.01){
                taugrad[o_idx] = 0;
                resgrad[o_idx] = 0;
                continue;
...
/*
* function: aggregate the grad generated by the ip rule
* blocks  : dim3(outputSize, outputAmount)
* threads : dim3(batch)
*/
__global__ void g_intrinsic_plasticity_gradadd(float* taugradTmp, float* taugrad, float* resgradTmp, float* resgrad, int batch, int outputArea, int outputSize)
{
    ...
    for(int i = 0; i < batch; i += blockDim.x)
    {
        ...
        if(b < batch)
        {
            _t_sum[tid] += taugradTmp[ok * outputArea + b * outputSize + o_idx];
            _r_sum[tid] += resgradTmp[ok * outputArea + b * outputSize + o_idx];
        }
    }
    int len = blockDim.x;
    while(len != 1)
    {
        ...
        {
            _t_sum[tid] += _t_sum[tid + skip];
            _r_sum[tid] += _r_sum[tid + skip];
        }
        len = skip;
    }
    if(tid == 0)
    {
        taugrad[ok * outputSize + o_idx] = _t_sum[0] / batch;
        resgrad[ok * outputSize + o_idx] = _r_sum[0] / batch;
    }
}

/*
* function: update the tau and r by the grad obtained by the ip rule
* blocks  = dim3(min((tauM->getLen() + 255)/256, 5120))
* threads = dim3(256)
*/
